...
- Issue #160 / D11.1: Preconditions fuer AI-assisted Strategy Coaching sind als reiner Konzeptschritt dokumentiert; Fakten, Nutzerannahmen, KI-Hypothesen, offene Fragen, Quellen-/Evidenzlogik, Speicherlogik nach Nutzerbestaetigung und D11-Folgephasen sind abgegrenzt, ohne Produkt-, Backend-, KI-, RAG-, Simulations- oder Persistenzlogik einzufuehren
...
28. D11.1 / #160: Preconditions fuer AI-assisted Strategy Coaching dokumentiert.
...
38. D13.1 / #170: Produktkante `Strategy -> Briefing Preparation` fachlich zugeschnitten und Simulation Preparation als spaeteren Folgeblock abgegrenzt.
...
D9.2 bestaetigt diesen Briefing-Preparation-Einstieg auf Hostinger-Staging: `/briefing` laedt auf `fd6b145`, Backend, Frontend, DB und Alembic Head sind gesund, Desktop und Mobile bleiben stabil, die Browser-Console ist unauffaellig und die Seite suggeriert weiterhin keine fertige KI-Briefing-Erzeugung, keine produktive Simulation und kein automatisches Trainerreview. D9.2 ist ein reiner Smoke-/Dokumentationsschritt ohne neuen Produktumfang.
...
D11.1 / Issue #160 dokumentiert diese Voraussetzungen als Konzept- und Preconditions-Schritt. Festgehalten sind insbesondere stabile manuelle Strategy-Objekte und -Flows, der projektbezogene KI-Kontext, die Trennung zwischen Fakten, Nutzerannahmen, KI-Hypothesen und offenen Fragen, Quellen-/Evidenzlogik, Zwischenspeicherung, Nutzerbestaetigung vor Speicherung sowie UX-Leitplanken fuer dialogische Strategieentwicklung. D11.1 fuehrt keine Produkt-, Backend-, KI-, RAG-, Simulations-, Trainerreview-, API-, Migrations- oder Persistenzlogik ein.

D11.2 / Issue #161 konkretisiert den fachlichen Kontextvertrag fuer diese spaetere projektbezogene KI-Nutzung. Dokumentiert sind die moeglichen Kontextbereiche Project, Company, User / Role, Request Item / Demand, Supplier, Existing Strategy, Procurement History, Knowledge / Evidence, Open Questions / Missing Information sowie Data Quality / Confidence. Zulaessige Datenquellen, getrennte Aussagearten, Quellen-/Evidenzmarker, Mindestqualitaet, Umgang mit unvollstaendigen oder widerspruechlichen Kontexten und ungeeignete Kontextbestandteile sind beschrieben. D11.2 bleibt ohne Produktcode, Backend, Frontend, KI-Service, RAG, API, Persistenz, Migration, Simulation oder Trainerreview-Logik.

D11.3 / Issue #162 konkretisiert das fachliche Quellen-, Claim- und Evidenzmodell fuer spaeteres Strategy Coaching. Dokumentiert sind moegliche Quellen wie Stammdatenobjekte, `NegotiationProject`, Strategy-Bausteine, `RequestItem`, `ProcurementHistoryItem`, `ImportJob`, `ImportRow`, `KnowledgeDocument`, spaetere `KnowledgeClaim`-Objekte, RAG-Snippets, Nutzerangaben und freigegebene Trainerhinweise. Claims sind als isolierbare Aussagen mit Ursprung abgegrenzt, nicht als ganze Dokumenttexte, ungepruefte KI-Ausgaben, automatisch wahre Fakten oder automatisch gespeicherte Strategy-Bausteine. Aussagearten, Evidence-/Confidence-Stufen, Aktualitaet, Herkunft, Widersprueche, zulaessige Nutzung im Coaching und ungeeignete Claims beziehungsweise Quellen sind beschrieben. D11.3 bleibt ohne Produktcode, Backend, Frontend, KI-Service, RAG, Claim-Extraktion, API, Persistenz, Migration, Simulation, Score-Engine oder Trainerreview-Logik.

D11.4 / Issue #163 konsolidiert den D11-Konzeptzwischenstand. D11.1 bis D11.3 sind als erledigtes Konzeptfundament eingeordnet, waehrend `docs/ai-assisted-strategy-coaching.md`, `docs/ai-strategy-context-contract.md`, `docs/ai-strategy-evidence-model.md`, `docs/roadmap.md` und `docs/codex-tasks.md` den aktuellen D11-Konzeptstand bilden. Die Implementierungsgrenze bleibt eindeutig: D11 ist kein kurzfristiger Umsetzungsblock; es gibt noch keinen AI Strategy Coach im Produkt, keine RAG- oder Claim-Extraktion, kein Persistenzmodell fuer KI-Dialoge, Hypothesen, bestaetigte Vorschlaege oder Reviewdaten und keine API-, Backend-, Frontend-, Datenbank- oder UI-Freigabe. Der bestehende Strategy Builder bleibt der aktuelle manuelle beziehungsweise regelbasierte Vorbereitungsbereich.

Spaetere D11-Folgeoptionen bleiben nur moegliche Reihenfolge, nicht sofortige Umsetzung: D11.5 kann Dialogphasen und UX-Vertrag spezifizieren, D11.6 ein Persistenz- und Reviewmodell fuer Entwuerfe, Bestaetigungen, Korrekturen und verworfene Vorschlaege definieren, D11.7 erst danach einen begrenzten technischen Prototyp pruefen. D11.4 fuehrt keine Produkt-, Backend-, Frontend-, KI-, RAG-, Claim-, API-, Persistenz-, Migrations-, Simulations-, Score-, Trainerreview- oder Staging-Logik ein.

Offene Nicht-Blocker nach D11.4:
...
D12.2 / Issue #165 fuehrt diese Matrix in einen technischen Demo-Seed-Plan in
`docs/demo-seed-plan.md` ueber. Der Plan ordnet den bestehenden
Rheinwerk-/Aurum-Demo-Seed ein, empfiehlt mehrere getrennte Demo-Projekte fuer
Empty Strategy, Partial Strategy, Ready Strategy, Supplier-Empty-State sowie
schwachen beziehungsweise gepflegten Supplier Context und beschreibt fuer diese
Zustaende betroffene Entitaeten, erwartete UI-Zustaende, Idempotenz,
lokale/Staging-Verfuegbarkeit und Risiken eines ueberladenen Hauptprojekts.
D12.2 bleibt ein reiner Planungs- und Dokumentationsschritt ohne Seed-,
Produkt-, Backend-, Frontend-, Migrations-, KI-, RAG-, Simulations-,
Trainerreview- oder Staging-Aenderung.

D12.3 / Issue #166 setzt den kleinsten D12-Demo-Satz im bestehenden
idempotenten Seed `backend/app/seeds/staging_demo.py` um. Der Seed legt
separate Demo-Projekte fuer RequestItem + SupplierProfile ohne Strategy,
unvollstaendige Strategy, teilweise gefuellte Strategy mit `Grundlage
vorhanden`, vollstaendige Strategy mit `Bereit fuer Briefing / Simulation` und
ein Projekt ohne SupplierProfile an. Die festen IDs und erwarteten Routen stehen
in `docs/demo-seed-plan.md`, `docs/demo-test-data-matrix.md` und
`docs/browser-smoke-test-plan.md`. D12.3 fuehrt kein Staging-Deployment, keine
...
D12.5 / Issue #168 bestaetigt dieselben D12.3-Demo-Zustaende auf
Hostinger-Staging. Staging wurde per Fast-Forward von `f2f444b` auf `d598988`
gebracht, der Stack neu gebaut und gestartet, Alembic `2f4b7c8d9e0a (head)`
geprueft und der idempotente Demo-Seed ausgefuehrt. Browserseitig wurden
Project Detail A-E, Preparation Gaps Card, Supplier Context vorhanden
beziehungsweise offen, Strategy Empty State, Readiness `Unvollstaendig`,
`Grundlage vorhanden` und `Bereit fuer Briefing / Simulation`,
Next-Action-Guidance, `/briefing?projectId=...`, Mobile-Darstellung und
Browser-Console bestanden geprueft. Das Ergebnis steht in
`docs/browser-smoke-test-plan.md`. D12.5 fuehrt keine Produktlogik, keine
...
D13.1 / Issue #170 entscheidet als reiner Konzept- und Scope-Schritt, dass D13
zuerst `Strategy -> Briefing Preparation` staerkt. Simulation Preparation
bleibt fachlich sinnvoll, wird aber nicht vorgezogen, weil sie staerker auf
spaetere Simulationslogik, Rollenverhalten, Gespraechsdynamik und Auswertung
aufsetzt. Der Briefing-Zwischenschritt kann dagegen direkt an vorhandene
Strategy Readiness, Preparation Gaps, Supplier Context und
`/briefing?projectId=...` anschliessen.

Der Scope ist in `docs/briefing-preparation-scope.md` dokumentiert. Als erste
Briefing-Bausteine sind Ausgangslage, Zielbild, Strategy Objectives, ZOPA,
BATNA, WAP, Konzessionslogik, Argumentationslinien, Lieferantenkontext,
Bedarfskontext aus `RequestItem`, optionale Hinweise aus `UserProfile`, offene
Informationsluecken und naechste Vorbereitungsschritte vorgesehen. Zulaessige
Datenquellen bleiben vorhandene Objekte: `NegotiationProject`, `RequestItem`,
`SupplierProfile`, `Strategy`, `ZopaItem`, `BatnaOption`, `ConcessionItem`,
`ArgumentationLine`, optional `UserProfile` sowie bestehende Readiness- und
Preparation-Guidance. D13.1 fuehrt keine Produktdateien, keine Frontend-UI,
keine Backend-API, keine Migration, keine Seed-Aenderung, kein
Staging-Deployment, keine KI-Briefing-Erzeugung, kein RAG, keine
